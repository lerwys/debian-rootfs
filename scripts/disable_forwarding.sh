...
sudo iptables -D POSTROUTING -t nat -o eth2 -j MASQUERADE
# Accept/Forward connections from LAN if they are already established from internal LAN
sudo iptables -D FORWARD -i eth2 -o enp5s0 -m state --state ESTABLISHED,RELATED -j ACCEPT
# Accept/Forward all outgoing internal LAN connections
sudo iptables -D FORWARD -i enp5s0 -o eth2 -j ACCEPT
